		...
		When("the URI path refers to a streaming RPC method", func() {
			DescribeTable(
				...
				Entry(
					...
					"the 'protean.test.TestService' service contains an RPC method named 'ClientStream', but it requires streaming and therefore must be called via a websocket connection",
				),
				Entry(
					"server streaming method",
					"/protean.test/TestService/ServerStream",
					"the 'protean.test.TestService' service contains an RPC method named 'ServerStream', but it requires streaming and therefore must be called via a websocket connection",
				),
				Entry(
					"bidirectional streaming method",
					"/protean.test/TestService/BidirectionalStream",
					"the 'protean.test.TestService' service contains an RPC method named 'BidirectionalStream', but it requires streaming and therefore must be called via a websocket connection",
				),
			)
		})
	})
